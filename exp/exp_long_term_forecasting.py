...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting, wandb=None):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            vali_loss = self.vali(vali_data, vali_loader, criterion, is_test=True, wandb=wandb) # NOTE( ): validation should be same as test since early stopping
            test_loss = self.vali(test_data, test_loader, criterion, is_test=True, wandb=wandb)
            if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
