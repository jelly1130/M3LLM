...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Main method setting: use a 4D causal mask over flattened
        # time-variable tokens. The mask lets a token attend to all
        # variables at the current and past patch positions, but not future
        # patch positions.
        self.use_multivariate_mask = getattr(configs, 'use_multivariate_mask', True)
        ...
        if self.mix:
            ...
            if self.mix_embeds_type in ['v8', 'v9', 'v10', 'v11', 'v12', 'v13']:
                del self.add_scale
                ...
                elif self.mix_embeds_type in ['v9', 'v11', 'v12', 'v13']:
                    self.hidden_vector_for_cross_list = nn.ParameterList()
                    for i in range(self.n_vars):
                        if self.mix_embeds_type in ['v9', 'v11']:
                            self.hidden_vector_for_cross_list.append(
                                nn.Parameter(torch.randn(self.n_vars, hidden_dim_for_cross, device=self.device))
                            )
                        elif self.mix_embeds_type in ['v12', 'v13']:
                            self.hidden_vector_for_cross_list.append(
                                nn.Parameter(torch.randn(self.n_vars, self.hidden_dim_of_llm, device=self.device))
                            )
                    if self.mix_embeds_type in ['v11', 'v12', 'v13']:
    ...
    def _target_aware_bi_interaction(self, times_embeds, bs, n_vars, token_num):
        ...
        for hidden_vector in self.hidden_vector_for_cross_list:
            # hidden_vector: [n_vars, hidden_dim], target-specific source weights
            weighted = source_embeds * hidden_vector.unsqueeze(0).unsqueeze(0)
            sum_square = torch.pow(torch.sum(weighted, dim=2), 2)
            square_sum = torch.sum(torch.pow(weighted, 2), dim=2)
            cross_output = 0.5 * (sum_square - square_sum)
            bie_outputs.append(cross_output.unsqueeze(2))

        # [bs, token_num, n_vars, hidden_dim] -> [bs * n_vars, token_num, hidden_dim]
        bie = torch.cat(bie_outputs, dim=2)
        bie = bie.permute(0, 2, 1, 3).reshape(bs * n_vars, token_num, hidden_dim)
        return bie

    def _fuse_token_embeddings(self, times_embeds, x_mark_enc, bs, n_vars, token_num):
        """
    ...
    def _build_multivariate_attention_mask(self, batch_size, token_num, n_vars, device, dtype):
        """
        Build a 4D additive attention mask for flattened [time, variable] tokens.
        A query at patch t can attend to all variables at patch <= t.
        Future patch positions are masked to avoid temporal leakage.
        """
        seq_len = token_num * n_vars
        time_index = torch.arange(token_num, device=device).repeat_interleave(n_vars)
        allowed = time_index.unsqueeze(0) <= time_index.unsqueeze(1)  # key_time <= query_time
    ...
    def _llm_forward(self, times_embeds, bs, n_vars):
        """
        Run the frozen LLM either with native per-variable causal attention
        or with the multivariate causal mask used by the main method.
        """
        token_num = times_embeds.shape[1]
        if (not self.use_multivariate_mask) or n_vars == 1:
            return self.llm(inputs_embeds=times_embeds, output_hidden_states=True).hidden_states[-1]

        hidden_dim = times_embeds.shape[-1]
        # [bs * n_vars, token_num, D] -> [bs, token_num * n_vars, D]
        llm_inputs = times_embeds.reshape(bs, n_vars, token_num, hidden_dim)
        llm_inputs = llm_inputs.permute(0, 2, 1, 3).reshape(bs, token_num * n_vars, hidden_dim)

        attention_mask = self._build_multivariate_attention_mask(
            batch_size=bs,
            token_num=token_num,
            n_vars=n_vars,
        ...
        # [bs, token_num * n_vars, D] -> [bs * n_vars, token_num, D]
        outputs = outputs.reshape(bs, token_num, n_vars, hidden_dim)
        outputs = outputs.permute(0, 2, 1, 3).reshape(bs * n_vars, token_num, hidden_dim)
        return outputs
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.mix:
            if self.mix_embeds_type in ['v1', 'v5', 'v6', 'v7', 'v8', 'v9', 'v10', 'v11', 'v12', 'v13']:
                if self.mix_embeds_type in ['v1', 'v6', 'v8', 'v9', 'v10', 'v11', 'v12', 'v13']:
                    ...
                    if self.mix_embeds_type == 'v1':
                        ...
                        times_embeds = self._fuse_token_embeddings(times_embeds, x_mark_enc, bs, n_vars, token_num)
                    elif self.mix_embeds_type in ['v8', 'v9', 'v10', 'v11']:
                        # times_embeds: [bs * n_vars x token_num x hidden_dim_of_llm]
                        ori_times_embeds = times_embeds
        ...
        outputs = self._llm_forward(times_embeds, bs, n_vars) # last hidden state
    ...
    def classification(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        x_mark_enc = x_mark_enc / x_mark_enc.norm(dim=2, keepdim=True)
        if __mydebug__: print(f'{x_mark_enc.shape=} {times_embeds.shape=} {self.multi_var=}')
        if self.mix_embeds_type in ['v12', 'v13']:
            times_embeds = self._fuse_token_embeddings(times_embeds, x_mark_enc, bs, n_vars, token_num)
        elif self.mix_embeds_type in ['v9', 'v10', 'v11']:
            # times_embeds: [bs * n_vars x token_num x hidden_dim_of_llm]
            ori_times_embeds = times_embeds
            times_embeds = times_embeds.reshape(bs, n_vars, token_num, -1) # [bs x n_vars x token_num x hidden_dim_of_llm]
            times_embeds = times_embeds.permute(0, 2, 3, 1) # [bs x token_num x hidden_dim_of_llm x n_vars]
