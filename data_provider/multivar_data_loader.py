...
class UEAloader(Dataset):
    def __init__(self, args, root_path, file_list=None, flag=None):
        self.short_dict = {
        ...
        self.root_path = root_path
        self.flag = flag
        self.read_data(root_path, file_list, flag)
        self.all_IDs = [x for x in range(self.samples.shape[0])]

    
    def read_data(self, root_path, file_list, flag):
        filename = os.path.join(root_path, f'{flag}.pt')
        data = torch.load(filename)
        self.samples, self.labels = data['samples'], data['labels']
        self.samples = self.samples.permute(0, 2, 1)
        self.max_seq_len = self.samples.shape[1]
        data_name = self.root_path.split('/')[-1]
        self.data_stamp = torch.load(os.path.join(self.root_path, f'{self.short_dict[data_name]}.{flag}.pt'))
    
    def __len__(self):
        return len(self.all_IDs)
    
    def __getitem__(self, index):
        return self.samples[index], self.labels[index], self.data_stamp[index], self.data_stamp[index]
...
class Dataset_Preprocess(Dataset):
    ...
    def __read_data__(self):
        ...
        self.n_vars = len(df_raw.columns) - 1
    ...
    def __getitem__(self, index):
        ...
        return seq_x_mark_list

    def __len__(self):
        return len(self.data_stamp)

class UEA_Dataset_Preprocess(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 scale=True, seasonal_patterns=None):
        self.seq_len = size[0]
        self.label_len = size[1]
        self.pred_len = size[2]
        self.token_len = self.seq_len - self.label_len
        self.token_num = self.seq_len // self.token_len
        self.flag = flag
        self.data_set_type = root_path.split('/')[-1]
        # init
        assert flag in ['train', 'test', 'val']
        type_map = {'train': 0, 'val': 1, 'test': 2}
        self.set_type = type_map[flag]
    ...
    def __read_data__(self):
        ...
        # self.data_stamp = [str(x) for x in self.data_stamp]
        # self.n_vars = len(df_raw.columns) - 1
        raw = torch.load(os.path.join(self.root_path, f'{self.flag}.pt'))
        B, C, T = raw['samples'].shape
        self.n_vars = C
        self.T = T
        self.B = B
    ...
    def __getitem__(self, index):
        start = datetime.datetime.fromtimestamp(0) # NOTE( ): UEA没有时间戳，自定义
        end = (start + datetime.timedelta(seconds=self.T-1)).strftime("%Y-%m-%d %H:%M:%S")
        # NOTE( ): 数位可能没对齐，补零对齐
        digits = len(str(self.n_vars))
        seq_x_mark_list = [f"This is the No.{str(i).zfill(digits)} Time Series from {start} to {end}" for i in range(self.n_vars)]
        return seq_x_mark_list
